//inline css fail hoti h to use the classes  
//this- current target element
// array or object ki copy (...) sparse array/obj
...
function findRowCOl(ele) {
    ...
    let rowId = parseInt(idArray[1]);    //converting it into integer coz string ki tra add hora tha
...
for (let i = 1; i <= 100; i++) {
    ...
    $("#columns").append(`<div class="column-name">${str}</div>`);   //abcd in col
    $("#rows").append(`<div class="row-name">${i}</div>`);  // no in row
}
...
$("#cells").scroll(function () {
    ...
    $("#columns").scrollLeft(this.scrollLeft);     //for moving columns headings
...
function loadNewSheet() {
    $("#cells").text("");          //cells ko khali
    for (let i = 1; i <= 100; i++) {           
        let row = $('<div class="cell-row"></div>');   //creating element
...
function addEventsToCells() {
    ...
    $(".input-cell").mousemove(function (event) {  //drag
        event.preventDefault();
        if (event.buttons == 1 && !event.ctrlKey) {  //1 clicked hai
...
function getTopBottomLeftRightCell(rowId, colId) {
    ...
    return [topCell, bottomCell, leftCell, rightCell];    //returning in the form of array
}
...
function unselectCell(ele, e, topCell, bottomCell, leftCell, rightCell) {
    if (e.ctrlKey && $(ele).attr("contenteditable") == "false") {    //ctrl press pe unselect ni hora tha
...
function selectCell(ele, e, topCell, bottomCell, leftCell, rightCell, mouseSelection) {
    if (e.ctrlKey || mouseSelection) {
        ...
        if (leftSelected) {
            leftCell.addClass("right-selected");  //selected
            $(ele).addClass("left-selected");     //current
        }
...
function addRemoveSelectFromFontStyle(data, property) {   //bold italic
    if (data[property]) {
...
//start and end position of mouse
...
$(".menu-selector").change(function (e) {
    let value = $(this).val();
    let key = $(this).attr("id");   //fontfamily
    if (key == "font-family") {
        $("#font-family").css(key, value);
    }
    if (!isNaN(value)) {                     //isNaN- is not a no.
        value = parseInt(value);
    }
    $(".input-cell.selected").css(key, value);
    // $(".input-cell.selected").each(function (index, data) {         //comman step
    //     let [rowId, colId] = findRowCOl(data);                      //data row column
    //     cellData[selectedSheet][rowId - 1][colId - 1][key] = value;                //cell data ko update
    // });
...
$("#bold").click(function (e) {
    setFontStyle(this, "bold", "font-weight", "bold");   //bold ke liye
...
function loadSheet() {
    ...
    for (let i of rowKeys) {
        ...
        for (let j of colKeys) {
            let colId = parseInt(j);
            let cell = $(`#row-${rowId + 1}-col-${colId + 1}`); // first cell that has changes
    ...
    $(".sheet-tab.selected").bind("contextmenu", function (e) {
        ...
        $(".sheet-delete").click(function (e) {
            ...
            $(".delete-button").click(function (e) {
                if (totalSheets > 1) {
                    $(".sheet-modal-parent").remove();
                    let keysArray = Object.keys(cellData);   //keys array extracted from obj
                    ...
                    // selectSheet($(".sheet-tab.selected")[0]);  //$this to this
                    totalSheets--;
                    saved = false;
                } else {
                }
